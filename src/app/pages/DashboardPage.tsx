...
// import { fetchAllCostData } from "../data/costApi";
...
import type { Service } from "../components/WorkloadOptimization";
import { fetchAllCostData } from "../data/costApi";
import type { CostConsumptionDetail } from "../data/costApi";
import {
  fetchAllRuntimeInvestigations,
  fetchAllCostInvestigations,
} from "../data/investigationsApi";
...
export default function DashboardPage() {
  ...
  const {
    user,
    signOut,
    subscriptions: azureSubs,
    subscriptionsLoading,
    subscriptionsError,
  } = useAuth();
  const [data, setData] = useState<Service[]>();
  const [costLookup, setCostLookup] = useState<Map<string, CostConsumptionDetail>>(new Map());
  const [isLoading, setIsLoading] = useState(true);
  const [isFetching, setIsFetching] = useState(false);
  const [isError, setIsError] = useState(false);
  const [runtimeInvestigationLookup, setRuntimeInvestigationLookup] = useState<Map<string, string>>(
    new Map(),
  );
  const [costInvestigationLookup, setCostInvestigationLookup] = useState<Map<string, string>>(
    new Map(),
  );

  const loadWorkloads = useCallback(async () => {
    if (azureSubs.length === 0) return;
    const subs: Subscription[] = azureSubs.map((s) => ({
      id: s.subscriptionId,
      name: s.displayName,
    }));
    ...
    setIsFetching(true);
    try {
      const [workloads, costs, runtimeInv, costInv] = await Promise.all([
        fetchAllWorkloadData(subs),
        fetchAllCostData(subs),
        fetchAllRuntimeInvestigations(subs),
        fetchAllCostInvestigations(subs),
      ]);
      setData(workloads);
      setCostLookup(costs);
      setRuntimeInvestigationLookup(runtimeInv);
      setCostInvestigationLookup(costInv);
    } catch {
      setIsError(true);
    } finally {
      setIsLoading(false);
      ...
      <div className="min-h-screen flex items-center justify-center bg-slate-100 p-6">
        <div className="max-w-sm text-center bg-white border border-slate-200 rounded-2xl p-8 shadow-sm">
          ...
          <p className="mt-1 text-sm text-slate-500">
            The workload API request failed. Please try again.
          </p>
          <button
            onClick={() => void loadWorkloads()}
            className="mt-5 inline-flex items-center justify-center rounded-xl bg-indigo-600 px-4 py-2 text-sm font-bold text-white hover:bg-indigo-500 transition-colors"
  ...
  return (
    <WorkloadOptimization
      ...
      runtimeInvestigationLookup={runtimeInvestigationLookup}
      costInvestigationLookup={costInvestigationLookup}
    />
  );
}
